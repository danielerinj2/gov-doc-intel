...
insert into public.tenant_data_policies (
    tenant_id,
    raw_image_retention_years,
    structured_data_retention_years,
    fraud_logs_retention_years,
    archival_strategy,
    purge_policy,
...
values (
    ...
    10,
    10,
    'COLD_STORAGE_AFTER_RETENTION_WINDOW',
    'ANONYMIZE_AFTER_EXPIRY',
    'PSEUDONYMIZED_AND_GATED',
    'SERVICE_DELIVERY_FRAUD_PREVENTION_AUDIT'
)
on conflict (tenant_id) do nothing;

insert into public.tenant_partition_configs (
    tenant_id,
    partition_mode,
    residency_region,
    region_cluster,
...
values ('dept-education', 'LOGICAL_SHARED', 'default', 'region-a', false, 'STANDARD')
on conflict (tenant_id) do nothing;

insert into public.operational_runbooks (
    tenant_id,
    event_type,
    severity,
    title,
